...
export default function InternalDashboardIndex() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      // Auto-redirect to user's specific dashboard
      const roleRoutes = {
  ...
  const getUserAccessibleRoles = () => {
    if (!user) return []

    // SuperAdmin can access all roles for demo purposes
    if (user.role === "SuperAdmin") {
      return roles
    }

    // Other users can only access their specific role
    return roles.filter((role) => role.roleKey === user.role)
  }
  ...
  const accessibleRoles = getUserAccessibleRoles()
    ...
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Next U Internal System</h1>
          <p className="text-xl text-gray-600">Select your dashboard to continue</p>
        </div>

        {accessibleRoles.length === 1 ? (
          // If user has only one role, show direct access
          <div className="text-center mb-8">
            <p className="text-lg text-gray-600 mb-6">You have access to:</p>
            <Link
              href={accessibleRoles[0].href}
              className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"
            >
              {React.createElement(accessibleRoles[0].icon, { className: "h-5 w-5 mr-2" })}
              Go to {accessibleRoles[0].name} Dashboard
            </Link>
          </div>
        ) : (
          // If user has multiple roles (SuperAdmin), show all options
